...
function handleCreateRoom(ws, msg) {
  ...
  const index = room.players.findIndex((x) => x === null);
  if (index === -1) return send(ws, { type: "ERROR", message: "Room is full" });

  room.players[index] = { ws, id: cryptoRandomId(), name, state: newPlayer() };
  room.hostIndex = index;

  send(ws, { type: "WELCOME", youIndex: index, code: room.code });
  broadcast(room, { type: "ROOM_STATE", state: snapshot(room) });
}
...
function handleJoinRoom(ws, msg) {
  ...
  room.players[index] = { ws, id: cryptoRandomId(), name, state: newPlayer() };
  if (room.hostIndex === null || room.hostIndex === undefined) room.hostIndex = index;
